...
"""CLI command for scraping SGLang and publishing inference metrics."""
...
@click.option(
    ...
    show_default=True,
    help="SGLang Prometheus metrics endpoint URL.",
)
@click.option(
    "--push-interval",
...
def main(
    ...
    """Scrape an SGLang server and publish inference metrics (throughput, TTFT, ITL, cache, queue)."""
    ...
    prev_state: Optional[sglang_monitor.CounterState] = None

    for _ in obj.looptimes(once):
        metrics, prev_state = sglang_monitor.scrape_sglang(
            sglang_endpoint,
            prev_state,
            float(push_interval),
        )
        ...
        logger.debug("Emitted SGLang metrics")
