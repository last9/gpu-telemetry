...
from l9gpu.monitoring.coerce import non_negative_int
from l9gpu.monitoring.date import BoundedClosedInterval, get_datetime
from l9gpu.monitoring.slurm.constants import TERMINAL_JOB_STATES
...
# when running l9gpu on containers, this is a known python issue:
...
def main(
    ...
    if cluster is None:
        cluster = os.environ.get("L9GPU_CLUSTER_NAME", "")
...
def new(
    ...
    Examples:
        ...
        l9gpu sacct_backfill \\
            --once \\
            new \\
            -s 'today 4pm' \\
            -e 'today 5pm' \\
            -- \\
            l9gpu sacct_publish -n

        # write to test category; verify incoming data
        # to verify incoming data
        l9gpu sacct_backfill --once new -s 'today 4pm' -e 'today 5pm' \\
            -- \\
            l9gpu sacct_publish \\
            --sink otlp \\
            -o app_secret=$APP_SECRET \\
            -o scribe_category=l9gpu_sacct_test
        ...
        l9gpu sacct_backfill --once new -s 'today 4pm' -e 'today 5pm' \\
            ...
            --sink otlp \\
            -o app_secret=$APP_SECRET \\
            -o scribe_category=l9gpu_sacct

        # write to actual category, recording errors in a file and retrying later
        l9gpu sacct_backfill --once new -s 'jan 1 2020 12am' -e 'dec 31 2020 11:59:59pm' \\
            -- \\
            l9gpu sacct_publish \\
            --sink otlp \\
            -o app_secret=$APP_SECRET \\
            -o scribe_category=l9gpu_sacct 2> errors-1.log
        ./my_error_postprocessing errors-1.log | l9gpu sacct_backfill --once from_file \\
            -- \\
            l9gpu sacct_publish \\
            --sink otlp \\
            -o app_secret=$APP_SECRET \\
            -o scribe_category=l9gpu_sacct 2> errors-2.log
...
def from_file(
    ...
    Examples:
        ...
        # dry run from STDIN
        cat intervals.csv | l9gpu sacct_backfill \\
            --once \\
            from_file \\
            -- \\
            l9gpu sacct_publish -n
        ...
        # write to test category; verify incoming data
        # to verify incoming data
        l9gpu sacct_backfill from_file --intervals intervals.csv \\
            --once \\
            -- \\
            l9gpu sacct_publish \\
            --sink otlp \\
