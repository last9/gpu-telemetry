...
"""Computes AMD GPU metrics averaged over a window and publishes metrics."""
...
from l9gpu.monitoring.device_telemetry_amd import AMDDeviceTelemetryClient, AMDGPUDevice
...
from l9gpu.schemas.amd_device_metrics import (
    AMDDeviceMetrics,
    AMDDevicePlusJobMetrics,
    IndexedAMDDeviceMetrics,
)
...
LOGGER_NAME = "amd_monitor"

log_error = error.log_error(logger_name=LOGGER_NAME)
logger: logging.Logger  # initialization in main()


def get_device_metrics_basic(handle: AMDGPUDevice) -> AMDDeviceMetrics:
    """Retrieve the AMD device metrics."""
    metrics = AMDDeviceMetrics(mem_used_percent=-1)
    ...
    metrics.xgmi_link_bandwidth = log_error(handle.get_xgmi_link_bandwidth)()
    metrics.ecc_per_block = log_error(handle.get_ecc_per_block)()
    metrics.junction_temperature = log_error(handle.get_junction_temperature)()
    metrics.hbm_temperature = log_error(handle.get_hbm_temperature)()
...
def compute_host_level_metrics(
    metrics: Iterable[AMDDeviceMetrics],
...
class CliObjectImpl:
    ...
    def get_device_telemetry(self) -> DeviceTelemetryClient:
        return AMDDeviceTelemetryClient()
...
def main(
    ...
    """Script for reading AMD GPU metrics on the node."""
    global logger
    ...
    for _ in obj.looptimes(once):
        ...
        accumulators = [
            Accumulator[AMDDeviceMetrics](max_fields(AMDDeviceMetrics))
        ...
        for index, metrics in enumerate(a.ask() for a in accumulators):
            indexed_device_metrics.append(
                IndexedAMDDeviceMetrics(gpu_index=index, **asdict(metrics))
            )

            device_plus_job_metrics = AMDDevicePlusJobMetrics(
                gpu_id=index,
                hostname=hostname,
                **asdict(metrics),
