...
from l9gpu.monitoring.coerce import ensure_dict
from l9gpu.monitoring.features.features_config import FeaturesConfig
from l9gpu.monitoring.passwd import Passwd
from l9gpu.monitoring.sink.utils import (
    Factory,
...
detach_option = click.option(
    ...
    is_flag=True,
    default=False,
    help=("Exit immediately instead of waiting for l9gpu to run. "),
)
...
cluster_option = click.option(
    "--cluster",
    help=(
        "Cluster name to attach as the `k8s.cluster.name` OTel resource attribute. "
        "Used for all deployment types (K8s, bare-metal, EC2) so that Last9 can group "
        "metrics by cluster consistently. If omitted, the attribute is not set."
    ),
)
...
def toml_config_option(
    name: str,
    *,
    default_config_path: Union[str, Path] = "/etc/l9gpu/config.toml",
