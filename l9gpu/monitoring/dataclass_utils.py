    ...
    def op(left: _TDataclass, right: _TDataclass) -> _TDataclass:
        ...
        for name in (f.name for f in fields(cls)):
            ...
            if left_value is None:
                ...
                # Dicts aren't orderable; merge per-key, taking max at overlapping keys.
                merged: dict = {**left_value}
                for k, v in right_value.items():
                    if k in merged and merged[k] is not None and v is not None:
                        merged[k] = max(merged[k], v)
                    elif v is not None:
                        merged[k] = v
                kwargs[name] = merged
            elif isinstance(left_value, list) and isinstance(right_value, list):
                # Python's built-in max(list, list) compares lexicographically
                # (picks the list with the larger first element), not
                # element-wise. For per-link / per-block metrics this silently
                # loses per-element peaks. Fall back to that behaviour only
                # when lengths differ (shape mismatch — shouldn't happen).
                if len(left_value) == len(right_value):
                    merged_list = []
                    for li, ri in zip(left_value, right_value):
                        if li is None:
                            merged_list.append(ri)
                        elif ri is None:
                            merged_list.append(li)
                        else:
                            merged_list.append(max(li, ri))
                    kwargs[name] = merged_list
                else:
                    kwargs[name] = right_value
            else:
