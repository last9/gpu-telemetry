...
Starting from  [`l9gpu/health_checks/cli/health_checks.py`](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/health_checks/cli/health_checks.py).
```python
...
def health_checks() -> None:
    """Last9 GPU Telemetry: Health checks for AI/HPC clusters."""
```
...
All checks live in [l9gpu/health_checks/checks/](https://github.com/last9/gpu-telemetry/tree/main/l9gpu/health_checks/checks) folder.
The [l9gpu/health_checks/](https://github.com/last9/gpu-telemetry/tree/main/l9gpu/health_checks) folder also contains other helper files such as [`l9gpu/health_checks/click.py`](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/health_checks/click.py) that defines our click related parameters and decorators.

Let's take the example of a single check to analyze. We'll analyze [check_storage.py](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/health_checks/checks/check_storage.py), and its `disk_usage()` command since it is also part of a subcommand group, but any check follows similar concepts.
...
There are two options for system commands in [subprocess.py](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/health_checks/subprocess.py). `shell_command()` that executes a single command, and `piped_shell_command()` that executes piped commands.
...
2. Implement unit tests to capture its behavior. Unit tests for the killswitches are in [l9gpu/tests/health_checks_tests/test_killswitches.py](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/tests/health_checks_tests/test_killswitches.py)

Using the same check as an example for the unit test, we can check file [l9gpu/tests/health_checks_tests/test_check_storage.py](https://github.com/last9/gpu-telemetry/blob/main/l9gpu/tests/health_checks_tests/test_check_storage.py).
...
def test_disk_usage(
    ...
    result = runner.invoke( # <-- Invoke the click command and pass the object
        check_storage,
        f"disk-usage test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -v /randomFolder --usage-critical-threshold=85 --usage-warning-threshold=80",
