...
from l9gpu.monitoring.cli.nvml_monitor import (
    CliObject,
    CliObjectImpl,
    get_ram_utilization,
    main,
...
from l9gpu.tests.fakes import FakeClock, FakeGPUDevice
...
def test_get_ram_utilization(monkeypatch: pytest.MonkeyPatch) -> None:
    # get_ram_utilization reads from psutil.virtual_memory() since the refactor
    # away from shell-command parsing; stub it with a namedtuple-lookalike.
    fake_mem = MagicMock()
    fake_mem.used = 22167
    fake_mem.total = 772649
    monkeypatch.setattr(
        "l9gpu.monitoring.cli.nvml_monitor.psutil.virtual_memory", lambda: fake_mem
    )

    actual = get_ram_utilization()

    assert abs(actual - (22167 / 772649)) < 1e-6
...
def test_cli(tmp_path: Path) -> None:
    ...
    # Utilization / percent fields are emitted as fractions (0-1), not percents.
...
def test_cli_run_interval(
    ...
    lines = result.stdout.strip().split("\n")
    assert len(lines) == 3
    actual_device_0 = json.loads(lines[0].split("- ")[2])
    for k, v in expected_device_0_info.items():
        assert actual_device_0[k] == v, f"Mismatch for key '{k}'"
    actual_device_1 = json.loads(lines[1].split("- ")[2])
    for k, v in expected_device_1_info.items():
        assert actual_device_1[k] == v, f"Mismatch for key '{k}'"
    actual_util = json.loads(lines[2].split("- ")[2])
    for k, v in expected_util_info.items():
        assert actual_util[k] == v, f"Mismatch for key '{k}'"
