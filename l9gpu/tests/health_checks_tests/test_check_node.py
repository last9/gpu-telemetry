...
from l9gpu.health_checks.checks.check_node import check_dnf_repos, check_module, uptime
...
def test_uptime(
    ...
    result = runner.invoke(
        uptime,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --uptime-threshold=600",
        obj=node_tester,
    )
...
def test_check_module(
    ...
    result = runner.invoke(
        check_module,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -m test_module --mod_count=1",
        obj=node_tester,
    )
...
def test_check_module_value_exception(
    ...
    with pytest.raises(ValueError):
        runner.invoke(
            check_module,
            f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -m m1 -m m2 --mod_count=1",
