...
from l9gpu.health_checks.check_utils.processor_memory_utils import (
    get_mem_attributes,
...
def test_processor_freq(
    ...
    result = runner.invoke(
        check_processor,
        f"processor-freq test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --proc_freq=1400",
        obj=proc_tester,
    )
...
def test_cpufreq_governor(
    ...
    result = runner.invoke(
        check_processor,
        f"cpufreq-governor test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --governor='performance'",
        obj=proc_tester,
    )
...
def test_check_mem_size(
    ...
    result = runner.invoke(
        check_processor,
        f"check-mem-size test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --dimms=32 --total-size=2048",
        obj=proc_tester,
    )
...
def test_check_buddyinfo(
    ...
    result = runner.invoke(
        check_processor,
        f"check-buddyinfo test_cluster prolog --log-folder={tmp_path} --sink=do_nothing",
        obj=proc_tester,
    )
...
def test_check_clocksource(
    ...
    args = f"check-clocksource test_cluster prolog --log-folder={tmp_path} --expected-source={expected_source} --sink=do_nothing"
