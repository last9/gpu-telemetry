...
from l9gpu.health_checks.check_utils.pynvml_errors import error_exit_codes
from l9gpu.health_checks.checks.check_nvidia_smi import (
...
from l9gpu.tests.fakes import FakeGPUDevice, FakeProcess
...
def test_check_gpu_num(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c gpu_num --gpu_num=8",
...
def test_check_app_clock_freq(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c clock_freq --gpu_app_freq=1155 --gpu_app_mem_freq=1593",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_running_procs(
    ...
    with (patch("psutil.pid_exists", return_value=True),):
        result = runner.invoke(
            check_nvidia_smi,
            f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c running_procs",
            obj=fake_nvidia_smi_obj,
        )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_pynvml_init_exception(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -c running_procs",
        obj=fake_nvidia_smi_obj,
    )
...
def test_check_and_kill_running_procs(
    ...
    with (
        ...
        result = runner.invoke(
            check_nvidia_smi,
            f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing --running_procs_force_kill {force_kill} -c running_procs_and_kill",
            obj=fake_nvidia_smi_obj,
        )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_gpu_temp(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c gpu_temperature --gpu_temperature_threshold=84",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_mem_usage(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c gpu_mem_usage --gpu_mem_usage_threshold=15",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_mem_usage_cli_env_devices_found(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -c gpu_mem_usage --gpu_mem_usage_threshold=15",
        obj=fake_nvidia_smi_obj,
        env={"CUDA_VISIBLE_DEVICES": "1"},
    )
    assert result.exit_code == 0
...
def test_check_gpu_retired_pages(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c gpu_retired_pages --gpu_retired_pages_threshold=10",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_ecc_uncorrected_volatile_total(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c ecc_uncorrected_volatile_total --ecc_uncorrected_volatile_threshold=0",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_ecc_corrected_volatile_total(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c ecc_corrected_volatile_total --ecc_corrected_volatile_threshold=10",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_row_remap(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c row_remap",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_row_remap_pending(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c row_remap_pending",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_check_row_remap_failed(
    ...
    result = runner.invoke(
        check_nvidia_smi,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c row_remap_failed",
        obj=fake_nvidia_smi_obj,
    )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
...
def test_vbios_mismatch(
    ...
    if vbios == "":
        result = runner.invoke(
            check_nvidia_smi,
            f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing -c vbios_mismatch",
            obj=fake_nvidia_smi_obj,
        )
    else:
        result = runner.invoke(
            check_nvidia_smi,
            f"test_cluster nagios --log-folder={tmp_path} --gpu_vbios {vbios} --sink=do_nothing -c vbios_mismatch",
            obj=fake_nvidia_smi_obj,
        )
    assert result.exit_code == expected[0].value
    assert expected[1] in caplog.text
