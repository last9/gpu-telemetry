...
from l9gpu.health_checks.checks.check_service import package_version, service_status
...
def test_service_status(
    ...
    result = runner.invoke(
        service_status,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -s randomService",
        obj=service_tester,
    )
...
def test_package_version(
    ...
    result = runner.invoke(
        package_version,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -p randomService -v 22.05.6-1.rsc.1",
        obj=service_tester,
    )
