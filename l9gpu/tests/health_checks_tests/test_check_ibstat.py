...
from l9gpu.health_checks.checks.check_ibstat import check_ib_interfaces, check_ibstat
...
def test_ibstat_physical_state(
    ...
    result = runner.invoke(
        check_ibstat,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --physical-state",
        obj=ibstat_tester,
    )
...
def test_ibstat_state(
    ...
    result = runner.invoke(
        check_ibstat,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --state",
        obj=ibstat_tester,
    )
...
def test_ib_interfaces_state(
    ...
    result = runner.invoke(
        check_ib_interfaces,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --interface-num=8",
        obj=ibstat_tester,
    )
