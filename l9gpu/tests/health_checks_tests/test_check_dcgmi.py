...
from l9gpu.health_checks.checks.check_dcgmi import check_dcgmi, DCGM, diag, nvlink
...
def test_process_dcgmi_diag_output(
    ...
    result = runner.invoke(
        diag,
        f"test_cluster nagios --log-folder={tmp_path} --sink=do_nothing",
...
def test_process_dcgmi_nvlink_error_output(
    ...
    result = runner.invoke(
        nvlink,
        f"test_cluster nagios --log-folder={tmp_path} -c nvlink_errors --data_error_threshold=10 --flit_error_threshold=10 --recovery_error_threshold=10 --replay_error_threshold=10 --sink=do_nothing",
        obj=dcgmi_shell_command_tester,
    )
...
def test_process_dcgmi_nvlink_status_output(
    ...
    result = runner.invoke(
        nvlink,
        f"test_cluster nagios --log-folder={tmp_path} -c nvlink_status --sink=do_nothing",
...
def test_exception_handling(
    ...
    result = runner.invoke(
        check_dcgmi,
        f"{check} test_cluster prolog --log-folder={tmp_path} --sink=do_nothing {check_params}",
        obj=fake_dcgm_obj,
    )
