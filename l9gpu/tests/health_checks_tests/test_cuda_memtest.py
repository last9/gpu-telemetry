...
from l9gpu.health_checks.checks.check_memtest import memtest
...
def test_cuda_memtest(
    ...
    result = runner.invoke(
        memtest,
        f"test_cluster prolog --log-folder={tmp_path} --size={size} --sink=do_nothing -gpu 0 -gpu 1",
...
def test_cuda_memtest_env_variables(
    ...
    result = runner.invoke(
        memtest,
        f"test_cluster prolog --log-folder={tmp_path} --size={size} --sink=do_nothing",
        obj=fake_cuda_memtest,
...
def test_cuda_memtest_exception(
    ...
    result = runner.invoke(
        memtest,
        f"test_cluster prolog --log-folder={tmp_path} --size=10 --sink=do_nothing -gpu 0",
        obj=fake_cuda_memtest,
    )
