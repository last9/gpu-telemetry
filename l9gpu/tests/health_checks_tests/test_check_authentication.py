...
from l9gpu.health_checks.checks.check_authentication import (
...
def test_pass_status(
    ...
    result = runner.invoke(
        password_status,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -u userA -s PS",
        obj=auth_tester,
    )
...
def test_check_path_access_by_user(
    ...
    result = runner.invoke(
        check_path_access_by_user,
        f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -u userA -p /path",
        obj=auth_tester,
    )
