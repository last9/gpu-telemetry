...
from l9gpu.health_checks.checks.check_storage import check_storage, process_disk_size
from l9gpu.health_checks.subprocess import PipedShellCommandOut, ShellCommandOut
...
def test_inode_usage(
    ...
    result = runner.invoke(
        check_storage,
        f"disk-usage test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -v /randomFolder --usage-critical-threshold=85 --usage-warning-threshold=80 --inode-check",
        obj=disk_usage_tester,
    )
...
def test_mounted_directory(
    ...
    result = runner.invoke(
        check_storage,
        f"mounted-directory test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -d /randomFolder",
        obj=mount_tester,
    )
...
def test_file_existance(
    ...
    result = runner.invoke(
        check_storage,
        f"file-exists test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -f /path/randomFile.txt",
...
def test_file_existance_should_not_exist(
    ...
    result = runner.invoke(
        check_storage,
        f"file-exists test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -f /path/randomFile.txt --should-not-exist",
        obj=existance_tester,
    )
...
def test_dir_existance(
    ...
    result = runner.invoke(
        check_storage,
        f"directory-exists test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -d /path/randomDir",
        obj=existance_tester,
    )
...
def test_check_mountpoint(
    ...
    result = runner.invoke(
        check_storage,
        f"check-mountpoint test_cluster prolog --log-folder={tmp_path} --sink=do_nothing --mountpoint=/checkpoint/",
...
def test_disk_size(
    ...
    args = f"disk-size test_cluster prolog --log-folder={tmp_path} --volume=/dev/sda3 --operator='>' --value={value} --size-unit=T --sink=do_nothing"
