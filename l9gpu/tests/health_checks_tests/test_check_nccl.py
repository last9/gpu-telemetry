...
from l9gpu.health_checks.checks.check_nccl import check_nccl, Flavor, get_hosts
from l9gpu.health_checks.subprocess import ShellCommandOut
...
def test_check_nccl_successful(
    ...
    args = f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -p all_reduce --nccl-tdir /opt/nccl-tests/build/ --critical-threshold {critical_threshold}"
...
def test_check_nccl_failure(tmp_path: Path) -> None:
    ...
    args = f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -p all_reduce --nccl-tdir /opt/nccl-tests/build/ --critical-threshold 200"
...
def test_nccl_exception(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        check_nccl,
        (
            f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -p all_reduce --nccl-tdir /opt/nccl-tests/build/ --critical-threshold 200"
        ),
        obj=mock_shell_command,
    )
...
def test_pairwise_nccl(
    ...
    args = f"test_cluster prolog --log-folder={tmp_path} --sink=do_nothing -p all_reduce --{flavor} --hostlist {hostlist} --nccl-tdir /opt/nccl-tests/build/ --critical-threshold 100"
